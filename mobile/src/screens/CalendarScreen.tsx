...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Alert, TextInput, Modal, Platform, Animated, Dimensions } from 'react-native';
...
const DEMO_EVENTS: CalendarEvent[] = [
    { id: '1', title: 'Revisión neurológica', date: '2026-03-10', time: '10:00', type: 'cita_medica', notes: 'Dr. Alejandro Vidal - Hospital La Fe', createdBy: 'familiar' },
    { id: '2', title: 'Análisis de sangre', date: '2026-03-05', time: '09:00', type: 'cita_medica', notes: 'Centro de salud - Ayunas', createdBy: 'cuidadora' },
    { id: '3', title: 'Visita de Laura', date: '2026-03-08', time: '17:00', type: 'visita', notes: 'Viene con los nietos', createdBy: 'familiar' },
    { id: '4', title: 'Revisión medicación', date: '2026-03-15', time: '11:30', type: 'medicamento', notes: 'Ajustar dosis de Memantina', createdBy: 'cuidadora' },
    { id: '5', title: 'Taller de estimulación', date: '2026-03-03', time: '10:00', type: 'actividad', notes: 'Centro de día', createdBy: 'cuidadora' },
    { id: '6', title: 'Fisioterapia', date: '2026-03-12', time: '16:00', type: 'actividad', createdBy: 'cuidadora' },
    { id: '7', title: 'Podólogo', date: '2026-02-28', time: '12:00', type: 'cita_medica', notes: 'Clínica podológica Alzira', createdBy: 'familiar' },
];
...
const CalendarScreen: React.FC<CalendarScreenProps> = ({ role }) => {
    ...
    const [newEvent, setNewEvent] = useState<Partial<CalendarEvent>>({ type: 'otro', time: '10:00' });
    ...
    useEffect(() => {
        (async () => {
            ...
            } catch (e) { console.warn('Error loading calendar events:', e); }
        })();
    }, []);
    ...
    const handleAddEvent = () => {
        ...
        const event: CalendarEvent = {
            id: Date.now().toString(), title: newEvent.title!.trim(), date: selectedDate,
            time: newEvent.time || '10:00', type: newEvent.type || 'otro',
            notes: newEvent.notes?.trim(), createdBy: role,
        };
        setEvents(prev => [...prev, event]);
        setShowAddModal(false);
        setNewEvent({ type: 'otro', time: '10:00' });
        calendarReminderService.scheduleReminders(event).then(() => {
            Alert.alert('🔔 Recordatorios activados', `Se notificará el día anterior y el mismo día del evento "${event.title}".`);
    ...
    const handleTestNotification = async () => {
        try {
            ...
            await Notif.scheduleNotificationAsync({
                content: { title: '🔔 Prueba de recordatorio', body: 'Las notificaciones del calendario funcionan correctamente.', sound: 'default' },
                trigger: { type: Notif.SchedulableTriggerInputTypes.TIME_INTERVAL, seconds: 3, repeats: false },
            });
    ...
    const renderDayCell = useCallback((day: number | null, index: number) => {
        ...
        const hasEvents = eventDates[dateStr], isWeekend = index % 7 >= 5;
        return (
            <TouchableOpacity key={`d-${day}`} style={{ width: DAY_SIZE, height: DAY_SIZE + 10, alignItems: 'center', justifyContent: 'center' }} onPress={() => setSelectedDate(dateStr)} activeOpacity={0.5}>
                <View style={[
                    { width: DAY_SIZE - 6, height: DAY_SIZE - 6, borderRadius: (DAY_SIZE - 6) / 2, alignItems: 'center', justifyContent: 'center' },
                    isSelected && { backgroundColor: colors.primary, ...SHADOWS.medium, shadowColor: colors.primary },
                    isToday && !isSelected && { backgroundColor: colors.primary + '15', borderWidth: 2, borderColor: colors.primary },
                ]}>
                    <Text style={[
                        { fontSize: 15, fontWeight: '500', color: isWeekend ? colors.textSecondary : colors.text },
                ...
                {hasEvents ? (
                    <View style={{ flexDirection: 'row', gap: 3, marginTop: 2 }}>
                        {hasEvents.slice(0, 3).map((t, j) => (
                            <View key={j} style={{ width: 5, height: 5, borderRadius: 2.5, backgroundColor: isSelected ? '#FFFFFF' : getEventTypeInfo(t).color }} />
                        ))}
                    </View>
                ) : <View style={{ height: 7 }} />}
            </TouchableOpacity>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
                <View style={[styles.monthNav, { backgroundColor: colors.card }, SHADOWS.small]}>
                    ...
                    <TouchableOpacity onPress={goToToday} style={styles.navCenter}>
                        <Text style={[styles.monthTitle, { color: colors.text }]}>{MONTHS[currentMonth]}</Text>
                    ...
                    <View style={styles.dayHeaderRow}>
                        {DAYS_SHORT.map((d, i) => (
                            <View key={d} style={{ width: DAY_SIZE, alignItems: 'center' }}>
                                <Text style={[styles.dayHeaderText, { color: i >= 5 ? colors.primary + '80' : colors.textSecondary }]}>{d}</Text>
                            </View>
                        ))}
                    </View>
                ...
                <View style={styles.eventsSection}>
                    ...
                    {selectedEvents.length === 0 ? (
                        <View style={[styles.emptyCard, { backgroundColor: colors.card }]}>
                            ...
                            <Text style={[styles.emptyTitle, { color: colors.textSecondary }]}>Sin eventos este día</Text>
                            <Text style={[styles.emptySubtitle, { color: colors.textLight }]}>Pulsa "Añadir" para crear uno</Text>
                        </View>
                    ) : (
                        selectedEvents.map(event => {
                            const typeInfo = getEventTypeInfo(event.type);
                            return (
                                <TouchableOpacity key={event.id} style={[styles.eventCard, { backgroundColor: colors.card }]} onLongPress={() => handleDeleteEvent(event.id)} activeOpacity={0.8}>
                                    ...
                                    <View style={[styles.eventTimeCol, { borderRightColor: colors.border + '40' }]}>
                                        <Text style={[styles.eventTimeHour, { color: typeInfo.color }]}>{event.time.split(':')[0]}</Text>
                                        <Text style={[styles.eventTimeMin, { color: typeInfo.color + 'CC' }]}>:{event.time.split(':')[1]}</Text>
                                    </View>
                                    <View style={styles.eventContent}>
                                        <View style={styles.eventTopRow}>
                                            ...
                                            <View style={[styles.creatorBadge, { backgroundColor: (event.createdBy === 'cuidadora' ? colors.primary : '#AB47BC') + '12' }]}>
                                                <Ionicons name={event.createdBy === 'cuidadora' ? 'person' : 'people'} size={10} color={event.createdBy === 'cuidadora' ? colors.primary : '#AB47BC'} />
                                                <Text style={[styles.creatorText, { color: event.createdBy === 'cuidadora' ? colors.primary : '#AB47BC' }]}>
                                                    {event.createdBy === 'cuidadora' ? 'Cuidadora' : 'Familiar'}
                                                </Text>
                                            </View>
                                        </View>
                                        <Text style={[styles.eventTitle, { color: colors.text }]}>{event.title}</Text>
                                        {event.notes ? <Text style={[styles.eventNotes, { color: colors.textSecondary }]} numberOfLines={2}>{event.notes}</Text> : null}
                                    </View>
                                </TouchableOpacity>
                            );
                        })
                ...
                {upcomingEvents.length > 0 && (
                    <View style={styles.upcomingSection}>
                        <View style={styles.upcomingSectionHeader}>
                            ...
                            <Text style={[styles.upcomingSectionTitle, { color: colors.text }]}>Próximos eventos</Text>
                        </View>
                        {upcomingEvents.map(event => {
                            ...
                            return (
                                <TouchableOpacity key={event.id} style={[styles.upcomingCard, { backgroundColor: colors.card }]}
                                    onPress={() => { setSelectedDate(event.date); setCurrentMonth(eventDate.getMonth()); setCurrentYear(eventDate.getFullYear()); }}
                                    activeOpacity={0.7}>
                                    ...
                                    <View style={styles.upcomingInfo}>
                                        ...
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 3 }}>
                                            ...
                                            <Text style={[styles.upcomingDate, { color: colors.textSecondary }]}>
                                                {isEventToday ? `Hoy · ${event.time}` : `${eventDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })} · ${event.time}`}
                                            </Text>
                                        </View>
                                    </View>
                ...
                <Text style={[styles.hint, { color: colors.textLight }]}>Mantén pulsado un evento para eliminarlo</Text>
            </ScrollView>
                ...
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
                        ...
                        <View style={styles.modalHeader}>
                            ...
                            <TouchableOpacity onPress={() => setShowAddModal(false)} style={[styles.modalClose, { backgroundColor: colors.inputBg }]}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <TextInput style={[styles.modalInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                placeholder="Ej: Revisión neurológica" placeholderTextColor={colors.textLight}
                                value={newEvent.title} onChangeText={t => setNewEvent(prev => ({ ...prev, title: t }))} />
                            <Text style={[styles.modalLabel, { color: colors.textSecondary }]}>Hora</Text>
                            <TextInput style={[styles.modalInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                placeholder="10:00" placeholderTextColor={colors.textLight}
                                value={newEvent.time} onChangeText={t => setNewEvent(prev => ({ ...prev, time: t }))}
                                ...
                                {EVENT_TYPES.map(t => {
                                    ...
                                    return (
                                        <TouchableOpacity key={t.value}
                                            style={[styles.typeCard, { borderColor: isActive ? t.color : colors.border + '60', backgroundColor: isActive ? t.color + '12' : colors.inputBg }]}
                                            onPress={() => setNewEvent(prev => ({ ...prev, type: t.value }))}>
                                            <View style={[styles.typeCardIcon, { backgroundColor: isActive ? t.color + '20' : colors.border + '30' }]}>
                                                <Ionicons name={t.icon as any} size={18} color={isActive ? t.color : colors.textLight} />
                                            </View>
                                            <Text style={[styles.typeCardLabel, { color: isActive ? t.color : colors.textSecondary }]}>{t.label}</Text>
                                        </TouchableOpacity>
                                    );
                                })}
                            </View>
                            <Text style={[styles.modalLabel, { color: colors.textSecondary }]}>Notas (opcional)</Text>
                            <TextInput style={[styles.modalInput, styles.modalTextarea, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                placeholder="Detalles adicionales..." placeholderTextColor={colors.textLight}
                                value={newEvent.notes} onChangeText={t => setNewEvent(prev => ({ ...prev, notes: t }))}
                                multiline numberOfLines={3} />
                            <TouchableOpacity style={[styles.saveButton, { backgroundColor: colors.primary }]} onPress={handleAddEvent} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
    ...
    monthStrip: {
        flexDirection: 'row', marginTop: 16, backgroundColor: 'rgba(255,255,255,0.12)',
    ...
    monthNav: {
        ...
        paddingVertical: 10, paddingHorizontal: 6, borderRadius: 18,
    },
    navArrow: {
    ...
    calendarCard: {
        ...
        borderRadius: 22, padding: SPACING.md, paddingBottom: SPACING.sm,
    },
    dayHeaderRow: { flexDirection: 'row', paddingBottom: 6 },
    ...
    legendRow: {
        ...
        gap: 12, marginHorizontal: SPACING.lg, marginTop: SPACING.md,
    },
    ...
    addButton: {
        flexDirection: 'row', alignItems: 'center', gap: 5,
        paddingHorizontal: 16, paddingVertical: 10, borderRadius: 14, ...SHADOWS.small,
    },
    addButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '700' },
    emptyCard: {
        alignItems: 'center', padding: SPACING.xl, borderRadius: 20, ...SHADOWS.small,
    },
    emptyIconWrap: {
    ...
    eventTimeCol: {
        ...
        borderRightWidth: 1, paddingVertical: SPACING.md,
    },
    ...
    upcomingCard: {
        ...
        borderRadius: 16, marginBottom: SPACING.sm, ...SHADOWS.small,
    },
    upcomingIconWrap: {
    ...
    modalContent: {
        ...
        paddingHorizontal: SPACING.lg, paddingBottom: Platform.OS === 'ios' ? 40 : SPACING.lg, maxHeight: '88%',
    },
    modalDrag: {
    ...
    modalHeader: {
        flexDirection: 'row', justifyContent: 'space-between',
        alignItems: 'flex-start', marginBottom: SPACING.md, paddingTop: 4,
    },
    ...
    modalLabel: {
        fontSize: 13, fontWeight: '700', marginBottom: 6, marginTop: 14,
        textTransform: 'uppercase', letterSpacing: 0.5,
    },
    modalInput: {
        borderRadius: 14, borderWidth: 1.5, paddingHorizontal: 16, paddingVertical: 14, fontSize: 16,
    },
    ...
    saveButton: {
        ...
        paddingVertical: 16, borderRadius: 16, marginTop: SPACING.lg, marginBottom: SPACING.md, ...SHADOWS.medium,
    },
