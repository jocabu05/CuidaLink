...
const InformeScreen: React.FC = () => {
    ...
    const weekStats = useMemo(() => {
        ...
        const lastWeek = eventos.filter(e => {
    ...
    // Activity distribution (this week)
    ...
    const dailyHeatmap = useMemo(() => {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const count = eventos.filter(e => new Date(e.timestamp).toDateString() === dateStr).length;
            days.push({
    ...
    const medAdherence = useMemo(() => {
        ...
        // Assume 2 per day * 7 = 14 expected
        const expected = 14;
        const pct = Math.min(Math.round((meds / expected) * 100), 100);
        return { count: meds, expected, pct };
    }, [eventos]);
    ...
    const insights = useMemo(() => {
        ...
        if (weekStats.diff > 0) {
            items.push({
                icon: 'trending-up',
                text: `Esta semana hay un ${weekStats.pct}% más de actividad que la anterior`,
                color: '#4CAF50',
            });
        } else if (weekStats.diff < 0) {
            items.push({
                icon: 'trending-down',
                text: `La actividad ha bajado un ${Math.abs(weekStats.pct)}% respecto a la semana pasada`,
                color: '#FF9800',
            });
        }
        ...
        if (medAdherence.pct >= 80) {
            items.push({
                ...
                text: `Excelente adherencia a la medicación: ${medAdherence.pct}%`,
                color: '#4CAF50',
            });
        } else if (medAdherence.count > 0) {
            items.push({
                icon: 'alert-circle',
                text: `La adherencia a medicamentos (${medAdherence.pct}%) puede mejorar`,
                color: '#FF9800',
            });
        }
        ...
        if (promedio >= 4) {
            items.push({
                icon: 'happy',
                text: `Valoración media excelente: ${promedio.toFixed(1)}/5`,
                color: '#4CAF50',
            });
        } else if (promedio >= 2.5) {
            items.push({
                icon: 'remove-circle',
                text: `Valoración media aceptable: ${promedio.toFixed(1)}/5`,
                color: '#FF9800',
            });
        } else if (promedio > 0) {
            items.push({
                icon: 'sad',
                text: `Valoración baja: ${promedio.toFixed(1)}/5 — atención recomendada`,
                color: '#F44336',
            });
        }
        ...
        if (bestDay && bestDay.count > 0) {
            items.push({
                icon: 'star',
                text: `Día más activo: ${bestDay.label} (${bestDay.date}) con ${bestDay.count} actividades`,
                color: '#1565C0',
            });
        }
        ...
        try {
            ...
            if (await Sharing.isAvailableAsync()) {
                await Sharing.shareAsync(uri, {
                    ...
                    dialogTitle: 'Compartir informe CuidaLink',
                    UTI: 'com.adobe.pdf',
                });
            } else {
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
                ...
                <View style={styles.comparisonRow}>
                    ...
                    <View style={[styles.compareCard, { backgroundColor: colors.card }]}>
                        ...
                        <View style={[styles.compareTrend, { backgroundColor: weekStats.diff >= 0 ? '#4CAF5015' : '#FF980015' }]}>
                            <Ionicons
                                name={weekStats.diff >= 0 ? 'arrow-up' : 'arrow-down'}
                                size={12}
                                color={weekStats.diff >= 0 ? '#4CAF50' : '#FF9800'}
                            />
                    ...
                    <View style={[styles.compareCard, { backgroundColor: colors.card }]}>
                        ...
                        <View style={[styles.compareTrend, { backgroundColor: weekStats.diasActivos >= weekStats.diasActivosPrev ? '#4CAF5015' : '#FF980015' }]}>
                            <Ionicons
                                name={weekStats.diasActivos >= weekStats.diasActivosPrev ? 'arrow-up' : 'arrow-down'}
                                size={12}
                                color={weekStats.diasActivos >= weekStats.diasActivosPrev ? '#4CAF50' : '#FF9800'}
                            />
                ...
                <View style={[styles.card, { backgroundColor: colors.card }]}>
                    ...
                    <View style={styles.chartContainer}>
                        {dailyHeatmap.map((day, idx) => {
                            const barH = Math.max(Math.round((day.count / maxDaily) * 100), 4);
                            const isToday = idx === 6;
                            return (
                                <View key={idx} style={styles.chartColumn}>
                                    <Text style={[styles.chartValue, { color: isToday ? colors.primary : colors.textSecondary }]}>
                                        {day.count}
                                    </Text>
                ...
                <View style={[styles.card, { backgroundColor: colors.card }]}>
                    ...
                    {Object.entries(TIPO_LABELS).map(([tipo, label]) => {
                        const count = distribution.byType[tipo] || 0;
                                        ...
                                        <View
                                            style={[
                                                styles.distBarFill,
                                                {
                                                    ...
                                                    backgroundColor: TIPO_COLORS[tipo] || colors.primary,
                                                },
                                            ]}
                                        />
                                    </View>
                ...
                <View style={[styles.card, { backgroundColor: colors.card }]}>
                    ...
                    <View style={styles.adherenceRow}>
                            ...
                            <View style={[styles.ringOuter, { borderColor: colors.border + '30' }]}>
                                <View
                                    style={[
                                        ...
                                        {
                                            borderColor:
                                                medAdherence.pct >= 80 ? '#4CAF50' :
                                                medAdherence.pct >= 50 ? '#FF9800' : '#F44336',
                        ...
                        <View style={styles.adherenceInfo}>
                            ...
                            <View
                                style={[
                                    styles.adherenceStatusBadge,
                                    {
                                        backgroundColor:
                                            medAdherence.pct >= 80 ? '#4CAF5015' :
                                            medAdherence.pct >= 50 ? '#FF980015' : '#F4433615',
                                    },
                                ]}
                            >
                                <Text
                        ...
                        {ratingTrend.map((day, idx) => {
                            ...
                            return (
                                <View key={idx} style={styles.ratingTrendCol}>
                                    ...
                                    {day.value > 0 ? (
                                        <Text style={[styles.ratingTrendVal, { color: isToday ? colors.primary : colors.textSecondary }]}>
                                            {day.value.toFixed(1)}
                                        </Text>
                                    ) : (
...
const styles = StyleSheet.create({
    ...
    circle1: { width: 200, height: 200, top: -60, right: -40, backgroundColor: 'rgba(255,255,255,0.08)' },
    circle2: { width: 140, height: 140, bottom: -30, left: -20, backgroundColor: 'rgba(255,255,255,0.06)' },
    headerContent: {
        flexDirection: 'row',
