...
const NotasScreen: React.FC<NotasScreenProps> = ({ role }) => {
    ...
    // Listen for when cuidadora marks a note as read
    useEffect(() => {
    ...
    const enviarNota = async () => {
        ...
        const titulo = prioridad === 'urgente'
            ? '🔴 NOTA URGENTE de la familia'
            : '📝 Nueva nota del familiar';
    ...
    // ─── VISTA FAMILIAR: Solo crear notas ───
    if (role === 'familiar') {
        return (
            <KeyboardAvoidingView
            ...
            >
                ...
                <View style={[styles.header, { backgroundColor: colors.headerBg }]}>
                    ...
                    <Text style={styles.headerSubtitle}>
                        Escribe instrucciones o avisos importantes
                    </Text>
                </View>
                        ...
                        <View style={styles.prioridadRow}>
                            <TouchableOpacity
                                style={[
                                    ...
                                    prioridad === 'normal' && { backgroundColor: colors.infoBg },
                                ]}
                            ...
                            >
                                <Text style={[
                                    ...
                                    prioridad === 'normal' && { color: colors.primary },
                                ]}>Normal</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={[
                                    styles.prioridadBtn,
                                    { backgroundColor: colors.inputBg },
                                    prioridad === 'urgente' && { backgroundColor: colors.dangerBg },
                                ]}
                            ...
                            >
                                <Text style={[
                                    styles.prioridadText,
                                    { color: colors.textSecondary },
                                    prioridad === 'urgente' && { color: colors.danger },
                                ]}>🔴 Urgente</Text>
                            </TouchableOpacity>
                        </View>
                        <TouchableOpacity
                            style={[
                                ...
                                (!texto.trim() || enviando) && styles.enviarDisabled,
                            ]}
                            onPress={enviarNota}
                ...
                <ScrollView
                    ...
                    refreshControl={
                        <RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); cargarNotas(); }} colors={[colors.primary]} />
                    }
                >
    ...
    // ─── VISTA CUIDADORA: Ver notas del familiar ───
    return (
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={[styles.header, { backgroundColor: colors.headerBg }]}>
                ...
                <Text style={styles.headerSubtitle}>
                    Lee las instrucciones y avisos
                </Text>
            </View>

            {!viendoTodas ? (
                /* Vista Principal: Solo nota más reciente */
                <ScrollView
                    style={styles.lista}
                    showsVerticalScrollIndicator={false}
                    refreshControl={
                ...
                >
                    ...
                    {notasFamiliar.length === 0 ? (
                        <View style={styles.emptyState}>
                            <Text style={styles.emptyEmoji}>📋</Text>
                            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
                                No hay notas del familiar.{'\n'}Aparecerán aquí cuando las cree.
                            </Text>
                        </View>
                    ) : (
                        <>
                            <TouchableOpacity
                                style={[
                                    ...
                                    notasFamiliar[0].leida && { opacity: 0.7 },
                                ]}
                                onPress={async () => {
                                    const nota = notasFamiliar[0];
                                    if (!nota.leida) {
                                        await notasService.marcarLeida(nota.id);
                                        cargarNotas();
                                    }
                                }}
                                activeOpacity={0.8}
                            ...
                            {notasFamiliar.length > 1 && (
                                ...
                                >
                                    <Text style={[styles.verMasText, { color: colors.primary }]}>
                                        👀 Ver todas ({notasFamiliar.length})
                                    </Text>
                                </TouchableOpacity>
                            )}
                        </>
                ...
                /* Vista Completa: Todas las notas con opciones */
                <ScrollView
                    style={styles.lista}
                    showsVerticalScrollIndicator={false}
                    refreshControl={
                ...
                >
                    <View style={styles.allNotasHeader}>
                        <Text style={[styles.listaTitle, { color: colors.text }]}>
                            Todas las notas ({notasFamiliar.length})
                        </Text>
                        <TouchableOpacity
                            style={[styles.volverBtn, { backgroundColor: colors.inputBg }]}
                            onPress={() => setViendoTodas(false)}
                        >
                        ...
                        <>
                            ...
                            {notasFamiliar.map((nota) => (
                                <View key={nota.id} style={styles.notaItemRow}>
                                    ...
                                    >
                                        <Text style={[styles.marcarBtnText, { color: colors.primary }]}>
                                            {nota.leida ? '✓' : '○'}
                                        </Text>
                                    </TouchableOpacity>
                                </View>
