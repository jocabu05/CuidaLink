...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, RefreshControl, Alert, Modal,
    TextInput, Switch, Animated, Dimensions, KeyboardAvoidingView, Platform } from 'react-native';
...
interface Medicamento {
    id: string; nombre: string; principioActivo: string; dosis: string;
    frecuencia: string; horarios: string[]; notas: string; color: string; recordatorio: boolean;
}

const DEFAULT_MEDS: Medicamento[] = [
    { id: 'med_sinemet', nombre: 'Sinemet', principioActivo: 'Levodopa/Carbidopa', dosis: '10/100 mg', frecuencia: '2 veces al día', horarios: ['10:30', '18:30'], notas: 'Tomar con alimentos. No mezclar con proteínas.', color: '#0277BD', recordatorio: true },
    { id: 'med_aricept', nombre: 'Aricept', principioActivo: 'Donepezilo', dosis: '5 mg', frecuencia: '1 vez al día', horarios: ['09:00'], notas: 'Tomar por la mañana en ayunas.', color: '#2E7D32', recordatorio: true },
    { id: 'med_memantina', nombre: 'Memantina', principioActivo: '', dosis: '10 mg', frecuencia: '1 vez al día', horarios: ['21:00'], notas: 'Tomar antes de dormir.', color: '#6A1B9A', recordatorio: true },
];
...
const MedicacionScreen: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            const [medsStr, evts] = await Promise.all([
                AsyncStorage.getItem(MEDS_KEY), localEventStorage.getEventos(),
            ]);
            if (medsStr) { setMedicamentos(JSON.parse(medsStr)); }
            else { setMedicamentos(DEFAULT_MEDS); await AsyncStorage.setItem(MEDS_KEY, JSON.stringify(DEFAULT_MEDS)); }
            setEventos(evts);
        } catch (e) {
    ...
    const openAddModal = () => {
        setEditingMed(null); setFormNombre(''); setFormPrincipio(''); setFormDosis('');
        setFormFrecuencia('1 vez al día'); setFormHorarios(''); setFormNotas('');
        setFormColor(FORM_COLORS[Math.floor(Math.random() * FORM_COLORS.length)]);
        setShowModal(true);
    };

    const openEditModal = (med: Medicamento) => {
        setEditingMed(med); setFormNombre(med.nombre); setFormPrincipio(med.principioActivo);
        setFormDosis(med.dosis); setFormFrecuencia(med.frecuencia); setFormHorarios(med.horarios.join(', '));
        setFormNotas(med.notas); setFormColor(med.color); setShowModal(true);
    };

    const handleSaveMed = async () => {
        if (!formNombre.trim() || !formDosis.trim() || !formHorarios.trim()) {
            Alert.alert('Campos obligatorios', 'Nombre, dosis y horarios son necesarios.'); return;
        }
        const horarios = formHorarios.split(',').map(h => h.trim()).filter(h => /^\d{1,2}:\d{2}$/.test(h));
        if (horarios.length === 0) {
            Alert.alert('Horario inválido', 'Escribe al menos un horario en formato HH:MM (ej: 09:00, 18:30)'); return;
        }
        const newMed: Medicamento = {
            id: editingMed?.id || generateId(), nombre: formNombre.trim(), principioActivo: formPrincipio.trim(),
            dosis: formDosis.trim(), frecuencia: formFrecuencia.trim() || `${horarios.length} vez${horarios.length > 1 ? 'es' : ''} al día`,
            horarios, notas: formNotas.trim(), color: formColor, recordatorio: editingMed?.recordatorio ?? true,
        };
        const updated = editingMed ? medicamentos.map(m => m.id === editingMed.id ? newMed : m) : [...medicamentos, newMed];
        await saveMeds(updated);
        if (newMed.recordatorio) {
            for (const hora of horarios) {
                try { await notificationService.programarRecordatorio(hora, `${newMed.nombre} ${newMed.dosis}`); } catch { }
            }
        }
        setShowModal(false);
    ...
    const handleDeleteMed = (med: Medicamento) => {
        Alert.alert('Eliminar medicamento', `¿Seguro que quieres eliminar ${med.nombre}?`, [
            { text: 'Cancelar', style: 'cancel' },
            { text: 'Eliminar', style: 'destructive', onPress: async () => { await saveMeds(medicamentos.filter(m => m.id !== med.id)); } },
        ]);
    };

    const toggleReminder = async (med: Medicamento) => {
        const updated = medicamentos.map(m => m.id === med.id ? { ...m, recordatorio: !m.recordatorio } : m);
        await saveMeds(updated);
        if (!med.recordatorio) {
        ...
        <View style={[styles.root, { backgroundColor: colors.background }]}>
            ...
            <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}
                            ...
                            <View key={`${item.med.id}_${item.hora}_${i}`} style={styles.timelineRow}>
                                ...
                                <View style={[styles.timelineContent,
                                    ...
                                    <View style={styles.timelineMedInfo}>
                                        <Text style={[styles.timelineMedName, { color: item.tomada ? colors.textLight : colors.text },
                                            item.tomada && { textDecorationLine: 'line-through' }]}>{item.med.nombre} {item.med.dosis}</Text>
                ...
                {medicamentos.map(med => {
                    ...
                    return (
                        <TouchableOpacity key={med.id} style={[styles.medCard, { backgroundColor: colors.card, borderLeftColor: med.color }]}
                            onPress={() => setExpandedMed(isExpanded ? null : med.id)} activeOpacity={0.7}>
                                ...
                                <View style={[styles.medExpanded, { borderTopColor: colors.border }]}>
                                    ...
                                    <View style={styles.medActions}>
                                        <View style={styles.medReminderToggle}>
                                            ...
                                            <Switch value={med.recordatorio} onValueChange={() => toggleReminder(med)}
                                        ...
                                        <View style={styles.medActionBtns}>
                                            <TouchableOpacity style={[styles.medActionBtn, { backgroundColor: colors.primary + '14' }]} onPress={() => openEditModal(med)}>
                                                <Ionicons name="create-outline" size={16} color={colors.primary} />
                                                <Text style={[styles.medActionBtnText, { color: colors.primary }]}>Editar</Text>
                                            </TouchableOpacity>
                                            <TouchableOpacity style={[styles.medActionBtn, { backgroundColor: colors.danger + '14' }]} onPress={() => handleDeleteMed(med)}>
                ...
                <TouchableOpacity style={[styles.addBtn, { borderColor: colors.primary }]} onPress={openAddModal} activeOpacity={0.7}>
                    ...
                    <View style={[styles.modalContainer, { backgroundColor: colors.card }]}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} style={styles.modalScroll}>
                            ...
                            <View style={styles.colorRow}>
                                {FORM_COLORS.map(c => (
                                    <TouchableOpacity key={c} style={[styles.colorPick, { backgroundColor: c, borderWidth: formColor === c ? 3 : 0, borderColor: '#fff' }]}
                            ...
                            <TextInput style={[styles.formInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                value={formNombre} onChangeText={setFormNombre} placeholder="Ej: Sinemet" placeholderTextColor={colors.textLight} />
                            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>Principio activo</Text>
                            <TextInput style={[styles.formInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                value={formPrincipio} onChangeText={setFormPrincipio} placeholder="Ej: Levodopa/Carbidopa" placeholderTextColor={colors.textLight} />
                            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>Dosis *</Text>
                            <TextInput style={[styles.formInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                value={formDosis} onChangeText={setFormDosis} placeholder="Ej: 10 mg" placeholderTextColor={colors.textLight} />
                            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>Frecuencia</Text>
                            <TextInput style={[styles.formInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                value={formFrecuencia} onChangeText={setFormFrecuencia} placeholder="Ej: 2 veces al día" placeholderTextColor={colors.textLight} />
                            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>Horarios * (separados por coma)</Text>
                            <TextInput style={[styles.formInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                value={formHorarios} onChangeText={setFormHorarios} placeholder="Ej: 09:00, 18:30" placeholderTextColor={colors.textLight} />
                            <Text style={[styles.formLabel, { color: colors.textSecondary }]}>Notas</Text>
                            <TextInput style={[styles.formInput, styles.formInputMulti, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
                                value={formNotas} onChangeText={setFormNotas} placeholder="Indicaciones especiales..."
                        ...
                        <View style={styles.modalActions}>
                            ...
                            <TouchableOpacity style={[styles.modalSaveBtn, { backgroundColor: colors.primary }]} onPress={handleSaveMed}>
...
const styles = StyleSheet.create({
    ...
    header: { paddingHorizontal: SPACING.lg, paddingTop: 52, paddingBottom: SPACING.lg,
        borderBottomLeftRadius: 28, borderBottomRightRadius: 28, overflow: 'hidden', ...SHADOWS.medium },
    ...
    adherenceCard: { borderRadius: 18, padding: 16, ...SHADOWS.small },
    ...
    timelineContent: { flex: 1, flexDirection: 'row', alignItems: 'center', marginLeft: 10, marginBottom: 10,
    ...
    medCard: { borderRadius: 18, padding: 16, marginBottom: 12, borderLeftWidth: 4, ...SHADOWS.small },
    ...
    addBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
        paddingVertical: 14, borderRadius: 16, borderWidth: 2, borderStyle: 'dashed', marginTop: 4 },
    addBtnText: { fontSize: 15, fontWeight: '700' },
    infoHint: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, padding: 14, borderRadius: 14, marginTop: 16 },
