...
const CL = {
    ...
    chatBgLight: '#F0F4FA',
    chatBgDark: '#121212',
    bubbleMineLight: '#D6E8FF',
    bubbleMineDark: '#1A3A5C',
    bubbleOtherLight: '#FFFFFF',
    bubbleOtherDark: '#1E1E1E',
    inputBgLight: '#FFFFFF',
    inputBgDark: '#2C2C2C',
    textPrimary: '#212121',
...
const ChatScreen: React.FC<ChatScreenProps> = ({ role }) => {
    ...
    // Map role to AsyncStorage key (cuidadora saves in PerfilAbuelo as 'abuelo')
    ...
    useEffect(() => {
        ...
        loadPhotos();
        // Refresh contact photo periodically (they may have changed it)
    ...
    // Pulse + slide-in animation for recording
    useEffect(() => {
        if (isRecording) {
        ...
        try {
            ...
            if (status !== 'granted') {
                if (!canAskAgain) {
                    Alert.alert(
                        'Permiso necesario',
                        'Se necesita acceso a la cámara. Actívalo en Ajustes.',
                        [
        ...
        try {
            ...
            if (status !== 'granted') {
                if (!canAskAgain) {
                    Alert.alert(
                        'Permiso necesario',
                        'Se necesita acceso a la galería. Actívalo en Ajustes.',
                        [
                            { text: 'Cancelar', style: 'cancel' },
                            { text: 'Abrir Ajustes', onPress: () => Linking.openSettings() },
                        ]
    ...
    const realizarLlamada = () => {
        Alert.alert(
            '📞 Llamar',
            `¿Quieres llamar ${role === 'cuidadora' ? 'al familiar' : 'a la cuidadora'}?`,
            [
                { text: 'Cancelar', style: 'cancel' },
                {
                    text: 'Llamar',
    ...
    const realizarVideollamada = () => {
        Alert.alert(
            '📹 Videollamada',
            `¿Quieres hacer una videollamada ${role === 'cuidadora' ? 'con el familiar' : 'con la cuidadora'}?`,
            [
                { text: 'Cancelar', style: 'cancel' },
                {
                    text: 'Videollamada',
                    onPress: () => {
                        // Abre FaceTime / app de videollamada del dispositivo
    ...
    const stopAndSendRecording = async () => {
        ...
        const msg = await chatService.enviarMensaje(
            role,
            `🎤 Nota de voz (${result.durationSeconds}s)`,
            ...
            >
                <View style={[styles.msgRow, isMine ? styles.msgRowMine : styles.msgRowOther]}>
                    ...
                    {isImage && item.imageBase64 ? (
                        <TouchableOpacity
                            activeOpacity={0.9}
                            onPress={() => setPreviewImage(`data:image/jpeg;base64,${item.imageBase64}`)}
                        >
                            <Image
                        ...
                        <TouchableOpacity style={styles.audioRow} onPress={() => playAudio(item)} activeOpacity={0.7}>
                            ...
                            <TouchableOpacity style={styles.playBtn} onPress={() => playAudio(item)}>
                                <Ionicons
                                    ...
                                    size={24}
                                    color={isMine ? CL.headerDark : CL.audioAccent}
                                />
                            </TouchableOpacity>
            ...
            <View style={[styles.header, { backgroundColor: colors.primary }]}>
                ...
                <View style={styles.headerInfo}>
                    <Text style={styles.headerName}>
                        {role === 'cuidadora' ? 'Familiar' : 'Cuidadora'}
                    </Text>
...
const styles = StyleSheet.create({
    container: {
        flex: 1,
        backgroundColor: CL.chatBgLight,
    },
    ...
    bubbleMine: {
        backgroundColor: CL.bubbleMineLight,
        borderTopRightRadius: 2,
    },
    bubbleOther: {
        backgroundColor: CL.bubbleOtherLight,
    ...
    tailMine: {
        ...
        borderLeftWidth: 6,
        borderRightWidth: 6,
        borderTopColor: CL.bubbleMineLight,
    ...
    tailOther: {
        ...
        borderLeftWidth: 6,
        borderRightWidth: 6,
        borderTopColor: CL.bubbleOtherLight,
        borderLeftColor: 'transparent',
        borderRightColor: 'transparent',
    },
    ...
    audioAvatar: {
        ...
        height: 36,
        borderRadius: 18,
        backgroundColor: CL.audioAccent,
        alignItems: 'center',
        justifyContent: 'center',
        overflow: 'hidden',
    },
    ...
    inputRow: {
        ...
        flexDirection: 'row',
        alignItems: 'flex-end',
        backgroundColor: CL.inputBgLight,
        borderRadius: 24,
        paddingHorizontal: 6,
