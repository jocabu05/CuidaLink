...
const ChatScreen: React.FC<ChatScreenProps> = ({ role }) => {
    ...
    const readFileAsBase64 = async (uri: string): Promise<string | null> => {
        try {
            ...
            return new Promise<string | null>((resolve) => {
                ...
                reader.onloadend = () => { const dataUrl = reader.result as string; resolve(dataUrl.split(',')[1] || null); };
    ...
    const enviarImagen = async (base64: string) => {
        setSending(true);
        const msg = await chatService.enviarMensaje(role, '📷 Foto', { type: 'image', imageBase64: base64 });
        setMensajes(prev => [...prev, msg]);
        setSending(false);
        scrollToEnd();
    };
    ...
    const cambiarFotoPerfil = async () => {
        Alert.alert('Foto de perfil', 'Elige una opción', [
            ...
            ...(profilePhoto ? [{ text: 'Eliminar foto', style: 'destructive' as const, onPress: async () => { setProfilePhoto(null); await AsyncStorage.removeItem(photoKey(role)); } }] : []),
    ...
    const stopAndSendRecording = async () => {
        ...
        setSending(true);
        Vibration.vibrate(50);
        const msg = await chatService.enviarMensaje(role, `🎤 Nota de voz (${result.durationSeconds}s)`, { type: 'audio', audioBase64: result.base64, audioDuration: result.durationSeconds });
        setMensajes(prev => [...prev, msg]);
        setSending(false);
        scrollToEnd();
    };
    ...
    const playAudio = async (msg: ChatMessage) => {
        if (playingId === msg.id) { await voiceService.stopPlayback(); setPlayingId(null); return; }
        if (msg.audioBase64) { setPlayingId(msg.id); await voiceService.playAudio(msg.audioBase64); setPlayingId(null); }
    };

    const confirmarEliminar = (msg: ChatMessage) => {
        Alert.alert('Eliminar mensaje', '¿Eliminar este mensaje?', [
            { text: 'Cancelar', style: 'cancel' },
            { text: 'Eliminar para todos', style: 'destructive', onPress: async () => { await chatService.eliminarMensaje(msg.id); setMensajes(prev => prev.filter(m => m.id !== msg.id)); }},
        ]);
    };
    ...
    const formatDuration = (secs: number) => { const m = Math.floor(secs / 60), s = secs % 60; return `${m}:${s.toString().padStart(2, '0')}`; };
        ...
        return (
            <TouchableOpacity activeOpacity={0.8} onLongPress={() => confirmarEliminar(item)} delayLongPress={600}>
                <View style={[styles.msgRow, isMine ? styles.msgRowMine : styles.msgRowOther]}>
                    <View style={[styles.bubble, isMine ? { backgroundColor: isDark ? CL.bubbleMineDark : CL.bubbleMineLight, borderTopRightRadius: 2 } : { backgroundColor: isDark ? CL.bubbleOtherDark : CL.bubbleOtherLight, borderTopLeftRadius: 2 }, isImage && styles.bubbleImage]}>
                    <View style={[styles.bubbleTail, isMine
                    ...
                    {!isMine && <Text style={[styles.senderLabel, { color: colors.primary }]}>{item.from === 'cuidadora' ? 'Cuidadora' : 'Familiar'}</Text>}
                    {isImage && item.imageBase64 ? (
                        ...
                        <TouchableOpacity style={styles.audioRow} onPress={() => playAudio(item)} activeOpacity={0.7}>
                            <View style={[styles.audioAvatar, isMine && styles.audioAvatarMine]}>
                                {isMine && profilePhoto ? <Image source={{ uri: profilePhoto }} style={{ width: 36, height: 36, borderRadius: 18 }} />
                                : !isMine && contactPhoto ? <Image source={{ uri: contactPhoto }} style={{ width: 36, height: 36, borderRadius: 18 }} />
                            ...
                            <View style={styles.waveformContainer}>
                                <View style={styles.waveform}>
                                    {heights.map((h, i) => (
                                        <View key={i} style={[styles.waveBar, { height: h }, { backgroundColor: isPlaying && i < 10 ? (isMine ? CL.headerDark : CL.audioAccent) : (isMine ? 'rgba(13,71,161,0.35)' : 'rgba(33,150,243,0.35)') }]} />
                                    ))}
                                </View>
                    ...
                    <View style={[styles.footer, isImage && styles.footerImage]}>
                        ...
                        {isMine && <Ionicons name="checkmark-done" size={16} color={isImage ? '#fff' : CL.audioAccent} style={{ marginLeft: 4 }} />}
                    </View>
                    </View>
                </View>
            </TouchableOpacity>
    ...
    return (
        <KeyboardAvoidingView style={[styles.container, { backgroundColor: isDark ? CL.chatBgDark : CL.chatBgLight }]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={90}>
            <View style={[styles.header, { backgroundColor: colors.primary }]}>
                <TouchableOpacity onPress={() => { if (contactPhoto) setPreviewImage(contactPhoto); }} activeOpacity={0.7}>
                    <View style={styles.headerAvatar}>
                        {contactPhoto ? <Image source={{ uri: contactPhoto }} style={styles.headerAvatarImg} /> : <Ionicons name="person" size={22} color="#fff" />}
                    </View>
                </TouchableOpacity>
            ...
            {mensajes.length === 0 ? (
                <View style={styles.emptyState}>
                    <View style={[styles.emptyBadge, { backgroundColor: isDark ? colors.surfaceElevated : '#E3F2FD' }]}>
                        ...
                        <Text style={[styles.emptyBadgeText, { color: colors.textSecondary }]}>Mensajes cifrados de extremo a extremo</Text>
                    </View>
                    <Text style={styles.emptyIcon}>💬</Text>
                    <Text style={[styles.emptySubtitle, { color: colors.textSecondary }]}>Envía un mensaje, foto o nota de voz</Text>
                </View>
            ) : (
                <FlatList ref={flatListRef} data={mensajes} renderItem={renderMessage} keyExtractor={item => item.id} contentContainerStyle={styles.messagesList} showsVerticalScrollIndicator={false} onContentSizeChange={() => flatListRef.current?.scrollToEnd({ animated: false })} />
            )}
            ...
            {isRecording && (
                <Animated.View style={[styles.recordBar, { backgroundColor: isDark ? colors.surface : '#fff', opacity: recordBarAnim, transform: [{ translateY: recordBarAnim.interpolate({ inputRange: [0, 1], outputRange: [50, 0] }) }] }]}>
            ...
            {!isRecording && (
                <View style={[styles.inputBar, { backgroundColor: isDark ? CL.chatBgDark : CL.chatBgLight }]}>
                    <View style={[styles.inputRow, { backgroundColor: isDark ? CL.inputBgDark : CL.inputBgLight }]}>
                        ...
                        <TextInput style={[styles.textInput, { color: isDark ? CL.textPrimaryDark : CL.textPrimary }]} value={texto} onChangeText={setTexto} placeholder="Mensaje" placeholderTextColor={colors.textSecondary} multiline maxLength={500} />
                        <TouchableOpacity style={styles.inputIcon} onPress={abrirGaleria}>
...
const styles = StyleSheet.create({
    ...
    header: { flexDirection: 'row', alignItems: 'center', backgroundColor: CL.headerMid, paddingTop: Platform.OS === 'ios' ? 54 : 36, paddingBottom: 10, paddingHorizontal: 10 },
    headerAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center', marginRight: 10, overflow: 'hidden' },
    ...
    emptyBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#E3F2FD', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 8, marginBottom: 24 },
    ...
    msgAvatar: { width: 28, height: 28, borderRadius: 14, backgroundColor: 'rgba(0,0,0,0.15)', alignItems: 'center', justifyContent: 'center', marginHorizontal: 3, overflow: 'hidden' },
    msgAvatarImg: { width: 28, height: 28, borderRadius: 14 },
    bubble: { maxWidth: '80%', borderRadius: 12, paddingHorizontal: 10, paddingTop: 6, paddingBottom: 4, position: 'relative' },
    ...
    bubbleTail: { position: 'absolute', top: 0, width: 0, height: 0, borderTopWidth: 10, borderBottomWidth: 0 },
    tailMine: { right: -6, borderLeftWidth: 6, borderRightWidth: 6, borderTopColor: CL.bubbleMineLight, borderLeftColor: 'transparent', borderRightColor: 'transparent' },
    tailOther: { left: -6, borderLeftWidth: 6, borderRightWidth: 6, borderTopColor: CL.bubbleOtherLight, borderLeftColor: 'transparent', borderRightColor: 'transparent' },
    ...
    footerImage: { position: 'absolute', bottom: 6, right: 8, backgroundColor: 'rgba(0,0,0,0.45)', borderRadius: 8, paddingHorizontal: 6, paddingVertical: 2 },
    timeText: { fontSize: 11, color: CL.timeText },
    timeImage: { color: '#fff' },
    audioRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 4, minWidth: 200 },
    audioAvatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: CL.audioAccent, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
    ...
    recordBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', paddingHorizontal: 12, paddingVertical: 10, paddingBottom: Platform.OS === 'ios' ? 28 : 10 },
    recordTrash: { padding: 8 },
    recordCenter: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
    recordDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: CL.recording, marginRight: 10 },
    recordTimer: { fontSize: 20, fontWeight: '600', color: CL.textPrimary, fontVariant: ['tabular-nums'] },
    recordSendBtn: { width: 48, height: 48, borderRadius: 24, backgroundColor: CL.accent, alignItems: 'center', justifyContent: 'center' },
    inputBar: { flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: 6, paddingVertical: 6, paddingBottom: Platform.OS === 'ios' ? 26 : 6, backgroundColor: CL.chatBgLight },
    inputRow: { flex: 1, flexDirection: 'row', alignItems: 'flex-end', backgroundColor: CL.inputBgLight, borderRadius: 24, paddingHorizontal: 6, paddingVertical: Platform.OS === 'ios' ? 6 : 2, minHeight: 46 },
    inputIcon: { padding: 6, justifyContent: 'center' },
    textInput: { flex: 1, fontSize: 16, color: CL.textPrimary, maxHeight: 100, paddingHorizontal: 4, paddingVertical: Platform.OS === 'ios' ? 8 : 6 },
    fabButton: { width: 48, height: 48, borderRadius: 24, backgroundColor: CL.headerMid, alignItems: 'center', justifyContent: 'center', marginLeft: 6 },
    previewOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.92)', justifyContent: 'center', alignItems: 'center' },
    previewClose: { position: 'absolute', top: Platform.OS === 'ios' ? 54 : 36, right: 16, zIndex: 10, padding: 8 },
