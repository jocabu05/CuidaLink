    ...
    const getMapHtml = () => {
        ...
        <body>
            ...
            <script>
                ...
                // ─── PROXIMITY MODE (viewport-based) ───
                var NEARBY_RADIUS = 800; // meters from map center
                var allMarkers = []; // {poi, marker, visible}
                var activeCats = {};
                ...
                function refreshVisiblePOIs() {
                    // Use map center so markers appear as you pan
                    var center = map.getCenter();
                    var refLat = center.lat;
                    var refLng = center.lng;
                    // Adapt radius to zoom: tighter to avoid overload
                    var zoom = map.getZoom();
                    var dynamicRadius = zoom >= 16 ? 400 : zoom >= 14 ? 700 : zoom >= 12 ? 1500 : 3000;
                    var MAX_VISIBLE = 15;
                    var nearbyN = 0;

                    // Sort by distance so closest POIs are prioritized
                    var scored = allMarkers.map(function(item) {
                        return { item: item, dist: haversineDistance(refLat, refLng, item.poi.lat, item.poi.lng) };
                    }).sort(function(a, b) { return a.dist - b.dist; });

                    var visibleCount = 0;
                    scored.forEach(function(entry) {
                        var item = entry.item;
                        var isNearby = entry.dist <= dynamicRadius;
                        var catOn = activeCats[item.poi.category];
                        var shouldShow = catOn && isNearby && visibleCount < MAX_VISIBLE;
                ...
                // ─── LAZY POPUP BUILDER (only on click, avoids heavy HTML upfront) ───
                function buildPopupHtml(poi, cat) {
                    var imgUrl = poi.image || catFallback[poi.category] || '';
                    var photoHtml = imgUrl
                        ? '<img src="' + imgUrl + '" class="popup-photo" loading="lazy" onerror="this.outerHTML=\\'<div class=popup-photo-placeholder>' + cat.emoji + '</div>\\';" />'
                ...
                function createMarkerForPoi(poi) {
                    var cat = catMap[poi.category];
                    if (!cat) return;
                    var shortName = poi.name.length > 14 ? poi.name.substring(0, 12) + '…' : poi.name;
                    var icon = L.divIcon({
                        className: '',
                        html: '<div class="poi-pin-wrap"><div class="poi-pin" style="background:' + cat.color + ';"><span class="poi-pin-emoji">' + cat.emoji + '</span></div><div class="poi-pin-label">' + shortName + '</div></div>',
                        iconSize: [42, 52],
                        iconAnchor: [21, 44],
                    });
                    var marker = L.marker([poi.lat, poi.lng], { icon: icon });
                    // Lazy popup: build HTML only on first click
                    var popupBound = false;
                    marker.on('click', function() {
                        if (!popupBound) {
                            marker.bindPopup(buildPopupHtml(poi, cat), { maxWidth: 290, closeButton: true }).openPopup();
                            popupBound = true;
                        }
                ...
                // ─── PROGRESSIVE POI LOADING: sorted by distance, in batches ───
                // Sort POIs nearest first so the user sees relevant ones immediately
                var refSortLat = userLat || HOME_LAT;
                var refSortLng = userLng || HOME_LNG;
                var sortedPois = pois.slice().sort(function(a, b) {
                    var dA = haversineDistance(refSortLat, refSortLng, a.lat, a.lng);
                    var dB = haversineDistance(refSortLat, refSortLng, b.lat, b.lng);
                    return dA - dB;
                });
                ...
                function loadNextBatch() {
                    ...
                    var end = Math.min(start + BATCH_SIZE, sortedPois.length);
                    if (start >= sortedPois.length) {
                        // All loaded — final refresh
                        refreshVisiblePOIs();
                        return;
                    }
                    for (var i = start; i < end; i++) {
                        createMarkerForPoi(sortedPois[i]);
                    }
                    batchIndex++;
                    // Refresh after each batch so nearby markers appear progressively
                    refreshVisiblePOIs();
                    // Schedule next batch — 120ms gives the browser time to breathe
                ...
                map.on('moveend', function() {
                    // Refresh markers based on new viewport center
                    refreshVisiblePOIs();
                ...
                <View style={styles.headerTop}>
                    <View style={{ flex: 1 }}>
                        ...
                        <Text style={[styles.subtitle, { color: isDark ? colors.textSecondary : 'rgba(255,255,255,0.8)' }]}>
                            {`📍 ${nearbyCount} lugar${nearbyCount !== 1 ? 'es' : ''} en esta zona`}
                        </Text>
                    </View>
