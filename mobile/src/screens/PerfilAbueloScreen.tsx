...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Linking, Alert, ActivityIndicator, TextInput, Keyboard, Switch, Image, Modal, Dimensions, Platform } from 'react-native';
...
const MEDS_DATA: MedicamentoInfo[] = [
...
const EXTRA_FIELDS = {
    edad: 82, contactoNombre: 'Laura Ruiz (hija)', diagnostico: 'Enfermedad de Alzheimer',
    fechaDiagnostico: 'Marzo 2021', faseAlzheimer: 'Fase 4 - Moderada', medicamentos: MEDS_DATA,
    alergias: ['Penicilina', 'Frutos secos'], grupoSanguineo: 'A+', pesoKg: 62, alturaCm: 158,
    condicionesSecundarias: ['Hipertensión arterial (controlada)', 'Osteoporosis leve', 'Hipoacusia bilateral'],
    ultimaVisitaMedica: '15 enero 2026', proximaCita: '10 marzo 2026',
    medico: 'Dr. Alejandro Vidal', centroSalud: 'Hospital La Fe, Valencia',
...
const FALLBACK_PERFIL: PerfilData = {
    nombre: 'Carmen Ruiz', direccion: 'C/Colmenar 59, Alzira', telefonoEmergencia: '961234567',
    notasMedicas: 'Sinemet 10mg - 10:30h y 18:30h\nAricept 5mg - mañanas',
    domicilioLat: 39.1512, domicilioLng: -0.4323, ...EXTRA_FIELDS,
};
...
const PerfilAbueloScreen: React.FC<PerfilAbueloScreenProps> = ({ abueloId = 1, onLogout }) => {
    ...
    const cargarPerfil = async () => {
        try {
            ...
            setPerfil({ ...(data as any), ...EXTRA_FIELDS });
        } catch {
            setPerfil(FALLBACK_PERFIL);
    ...
    const startEditing = useCallback((field: string, currentValue: string) => {
        setEditingField(field); setEditValue(currentValue);
    }, []);

    const saveEdit = useCallback(() => {
        if (!perfil || !editingField) return;
        setPerfil({ ...perfil, [editingField]: editValue });
    ...
    const pickerOpts = { mediaTypes: ['images'] as ImagePicker.MediaType[], quality: 0.5, allowsEditing: true, aspect: [1, 1] as [number, number], base64: true };
    const permDeniedAlert = () => Alert.alert('Permiso necesario', 'Actívalo en Ajustes.', [{ text: 'Cancelar', style: 'cancel' }, { text: 'Abrir Ajustes', onPress: () => Linking.openSettings() }]);

    const cambiarFotoPerfil = async () => {
        Alert.alert('Foto de perfil', 'Elige una opción', [
            { text: 'Cámara', onPress: async () => {
                const { status, canAskAgain } = await ImagePicker.requestCameraPermissionsAsync();
                if (status !== 'granted') { if (!canAskAgain) permDeniedAlert(); return; }
                handlePickResult(await ImagePicker.launchCameraAsync(pickerOpts));
            }},
            { text: 'Galería', onPress: async () => {
                const { status, canAskAgain } = await ImagePicker.requestMediaLibraryPermissionsAsync();
                if (status !== 'granted') { if (!canAskAgain) permDeniedAlert(); return; }
                handlePickResult(await ImagePicker.launchImageLibraryAsync(pickerOpts));
            }},
    ...
    const renderEditableBlock = (label: string, field: string, fieldValue: string, lines: number) => (
        <View style={styles.editableBlock}>
            <View style={styles.editableHeader}>
                <Text style={[styles.subSectionTitle, { color: colors.text, marginTop: 0, marginBottom: 0 }]}>{label}</Text>
                <TouchableOpacity onPress={() => editingField === field ? saveEdit() : startEditing(field, fieldValue)} style={[styles.editPill, { backgroundColor: colors.infoBg }]}>
                    <Ionicons name={editingField === field ? 'checkmark' : 'pencil'} size={13} color={colors.info} />
                    <Text style={[styles.editPillText, { color: colors.info }]}>{editingField === field ? 'Guardar' : 'Editar'}</Text>
                </TouchableOpacity>
            </View>
            {editingField === field ? (
                <View>
                    <TextInput style={[styles.editTextArea, { backgroundColor: isDark ? colors.surface : colors.background, color: colors.text, borderColor: colors.primary }]} value={editValue} onChangeText={setEditValue} multiline numberOfLines={lines} textAlignVertical="top" autoFocus />
                ...
                <Text style={[styles.observationText, { color: colors.text, backgroundColor: isDark ? colors.surface : colors.background }]}>{fieldValue}</Text>
            )}
        </View>
    );
    ...
    return (
        <View style={[styles.root, { backgroundColor: colors.background }]}>
            <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" onScrollBeginDrag={Keyboard.dismiss}>
                <View style={[styles.header, { backgroundColor: colors.headerBg }]}>
                    ...
                    <TouchableOpacity onPress={() => profilePhoto ? setPreviewPhoto(profilePhoto) : cambiarFotoPerfil()} onLongPress={cambiarFotoPerfil} delayLongPress={500} activeOpacity={0.8} style={styles.avatarWrapper}>
                        <View style={styles.avatarRing}>
                            <View style={styles.avatar}>
                                {profilePhoto ? <Image source={{ uri: profilePhoto }} style={styles.avatarImage} /> : <Text style={styles.avatarInitials}>{initials}</Text>}
                            </View>
                        </View>
                        ...
                        <View key={i} style={[styles.medCard, { backgroundColor: colors.card }]}>
                            ...
                            <View style={styles.medBody}>
                                <View style={styles.medHeaderRow}>
                                    <View style={[styles.medIconWrap, { backgroundColor: accent + '18' }]}><Ionicons name="medkit" size={16} color={accent} /></View>
                                ...
                                <View style={styles.medSchedule}>
                                    ...
                                    {med.horarios.map((h, j) => (
                                        <View key={j} style={[styles.timePill, { backgroundColor: accent + '14' }]}><Text style={[styles.timePillText, { color: accent }]}>{h}</Text></View>
                                    ))}
                                </View>
                                <Text style={[styles.medNotes, { color: colors.textSecondary }]}><Ionicons name="information-circle-outline" size={12} color={colors.textSecondary} />{'  '}{med.notas}</Text>
                            </View>
                        </View>
                    );
                })}
                ...
                <TouchableOpacity style={[styles.expandBtn, { backgroundColor: colors.card }]} onPress={() => setShowMedicalDetails(!showMedicalDetails)} activeOpacity={0.7}>
                    <Ionicons name={showMedicalDetails ? 'chevron-up-circle' : 'chevron-down-circle'} size={20} color={colors.info} />
                    <Text style={[styles.expandBtnText, { color: colors.info }]}>{showMedicalDetails ? 'Ocultar detalles médicos' : 'Ver más detalles médicos'}</Text>
                </TouchableOpacity>
                {showMedicalDetails && (
                    <View style={[styles.expandedBlock, { backgroundColor: colors.card }]}>
                        ...
                        <View style={[styles.doctorRow, { backgroundColor: isDark ? colors.surface : colors.background }]}>
                            <View style={[styles.doctorAvatar, { backgroundColor: colors.infoBg }]}><Ionicons name="person" size={20} color={colors.info} /></View>
                            ...
                            <TouchableOpacity style={[styles.doctorCallBtn, { backgroundColor: colors.info }]} onPress={llamarMedico}><Ionicons name="call" size={16} color="#fff" /></TouchableOpacity>
                        </View>
                        ...
                        {renderEditableBlock('Observaciones', 'observaciones', perfil.observaciones, 5)}
                        {renderEditableBlock('Notas adicionales', 'notasMedicas', perfil.notasMedicas, 4)}
                    </View>
                )}
                ...
                <View style={[styles.card, { backgroundColor: colors.card }]}>
                    <View style={styles.settingRow}>
                        ...
                        <Switch value={isDark} onValueChange={toggleDarkMode} trackColor={{ false: '#E0E0E0', true: colors.primary + '60' }} thumbColor={isDark ? colors.primary : '#FAFAFA'} />
                    </View>
                </View>
                <SectionHeader icon="call" title="Contactos de emergencia" colors={colors} />
                <TouchableOpacity style={[styles.contactRow, { backgroundColor: colors.card }]} onPress={llamarEmergencia} activeOpacity={0.7}>
                    <View style={[styles.contactAvatar, { backgroundColor: colors.infoBg }]}><Ionicons name="people" size={20} color={colors.info} /></View>
                    ...
                    <View style={[styles.contactCallBtn, { backgroundColor: colors.info }]}><Ionicons name="call" size={16} color="#fff" /></View>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.contactRow, { backgroundColor: colors.card, marginTop: 8 }]} onPress={llamarMedico} activeOpacity={0.7}>
                    <View style={[styles.contactAvatar, { backgroundColor: '#E8F5E9' }]}><Ionicons name="medkit" size={20} color="#43A047" /></View>
                    <View style={styles.contactInfo}>
                    ...
                    <View style={[styles.contactCallBtn, { backgroundColor: '#43A047' }]}><Ionicons name="call" size={16} color="#fff" /></View>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.contactRow, { backgroundColor: colors.card, marginTop: 8 }]} onPress={llamar112} activeOpacity={0.7}>
                    <View style={[styles.contactAvatar, { backgroundColor: colors.dangerBg }]}><Ionicons name="warning" size={20} color={colors.danger} /></View>
                    <View style={styles.contactInfo}>
                    ...
                    <View style={[styles.contactCallBtn, { backgroundColor: colors.danger }]}><Text style={styles.sosText}>SOS</Text></View>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.logoutBtn, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={() => Alert.alert('Cerrar sesión', '¿Seguro que quieres salir?', [{ text: 'Cancelar', style: 'cancel' }, { text: 'Salir', style: 'destructive', onPress: onLogout }])} activeOpacity={0.7}>
            ...
            <Modal visible={!!previewPhoto} transparent animationType="fade">
                <View style={styles.previewOverlay}>
                    <TouchableOpacity style={styles.previewClose} onPress={() => setPreviewPhoto(null)}><Ionicons name="close-circle" size={36} color="#fff" /></TouchableOpacity>
...
const SectionHeader = ({ icon, title, colors, count }: { icon: string; title: string; colors: any; count?: number }) => (
    <View style={styles.sectionHeader}>
        ...
        {count !== undefined && <View style={[styles.countBadge, { backgroundColor: colors.primary + '18' }]}><Text style={[styles.countBadgeText, { color: colors.primary }]}>{count}</Text></View>}
    </View>
);
...
const styles = StyleSheet.create({
    ...
    header: { paddingTop: Platform.OS === 'ios' ? 60 : 48, paddingBottom: 36, borderBottomLeftRadius: 32, borderBottomRightRadius: 32, alignItems: 'center', overflow: 'hidden' },
    headerCircle1: { position: 'absolute', width: 220, height: 220, borderRadius: 110, top: -60, right: -40 },
    headerCircle2: { position: 'absolute', width: 160, height: 160, borderRadius: 80, bottom: -30, left: -30 },
    headerTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', paddingHorizontal: 20, marginBottom: 20 },
    headerTitle: { fontSize: 16, fontWeight: '600', color: 'rgba(255,255,255,0.9)', letterSpacing: 0.5 },
    avatarWrapper: { position: 'relative', marginBottom: 14 },
    avatarRing: { width: 106, height: 106, borderRadius: 53, borderWidth: 3, borderColor: 'rgba(255,255,255,0.35)', alignItems: 'center', justifyContent: 'center' },
    avatar: { width: 94, height: 94, borderRadius: 47, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
    avatarImage: { width: 94, height: 94, borderRadius: 47 },
    avatarInitials: { fontSize: 34, fontWeight: '700', color: '#fff' },
    cameraBadge: { position: 'absolute', bottom: 4, right: 4, width: 28, height: 28, borderRadius: 14, backgroundColor: '#1565C0', alignItems: 'center', justifyContent: 'center', borderWidth: 2.5, borderColor: '#fff' },
    onlineDot: { position: 'absolute', top: 6, right: 10, width: 14, height: 14, borderRadius: 7, backgroundColor: '#66BB6A', borderWidth: 2.5, borderColor: '#fff' },
    ...
    statsCard: { flexDirection: 'row', marginHorizontal: 20, marginTop: -22, borderRadius: 18, paddingVertical: 14, paddingHorizontal: 8, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 6 },
    ...
    card: { marginHorizontal: 20, borderRadius: 16, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 3 },
    diagAccent: { position: 'absolute', left: 0, top: 12, bottom: 12, width: 4, borderRadius: 2 },
    diagBody: { paddingLeft: 8 },
    diagBadge: { flexDirection: 'row', alignSelf: 'flex-start', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, marginBottom: 8 },
    ...
    medCard: { marginHorizontal: 20, borderRadius: 16, marginBottom: 10, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 3 },
    medAccent: { height: 3, width: '100%' },
    medBody: { padding: 14 },
    medHeaderRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
    medIconWrap: { width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center', marginRight: 10 },
    medNameBlock: { flex: 1 },
    ...
    allergyChip: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, borderWidth: 1 },
    allergyText: { fontSize: 14, fontWeight: '600' },
    expandBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginHorizontal: 20, marginTop: 16, paddingVertical: 14, borderRadius: 14, gap: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 3 },
    expandBtnText: { fontSize: 15, fontWeight: '600' },
    expandedBlock: { marginHorizontal: 20, marginTop: 10, borderRadius: 16, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 3 },
    ...
    doctorAvatar: { width: 42, height: 42, borderRadius: 21, alignItems: 'center', justifyContent: 'center' },
    doctorInfo: { flex: 1, marginLeft: 10 },
    doctorName: { fontSize: 15, fontWeight: '600' },
    doctorCenter: { fontSize: 13, marginTop: 2 },
    doctorCallBtn: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
    ...
    editableHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
    editPill: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, gap: 4 },
    editPillText: { fontSize: 13, fontWeight: '600' },
    editTextArea: { borderRadius: 12, padding: 12, fontSize: 14, lineHeight: 22, minHeight: 100, borderWidth: 2 },
    ...
    contactRow: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 20, borderRadius: 14, padding: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 3 },
    ...
    contactCallBtn: { width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center' },
    sosText: { fontSize: 12, fontWeight: '800', color: '#fff' },
    logoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginHorizontal: 20, marginTop: 28, paddingVertical: 16, borderRadius: 14, borderWidth: 1, gap: 8 },
